...
impl DeploymentHistory {
    ...
    pub fn clear_deployment_history(&mut self, deployment: &str) -> bool {
        self.deployments.remove(deployment).is_some()
    }
...
pub fn clear_deployment_history(path: &str, deployment: Option<&str>) -> Result<()> {
    let mut history = match DeploymentHistory::load(path) {
        ...
        Err(e) => {
            error!("Ошибка загрузки истории деплоев: {}", e);
            return Err(anyhow::anyhow!("Не удалось загрузить историю деплоев"));
        }
    };

    match deployment {
        Some(name) => {
            if history.clear_deployment_history(name) {
                info!("История деплоя '{}' очищена", name);
            } else {
                warn!("История деплоя '{}' не найдена", name);
            }
        }
        None => {
            history.clear_all_history();
            info!("Вся история деплоев очищена");
        }
    }
