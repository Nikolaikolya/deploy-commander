...
impl Config {
	pub fn load(path: &str) -> Result<Self> {
		...
		if !config_path.exists() {
			...
			let config = Config { deployments: vec![] };
			config.save(path)?;
			return Ok(config);
		}
	...
	pub fn save(&self, path: &str) -> Result<()> {
		let yaml = serde_yaml::to_string(self)
			.context("Не удалось сериализовать конфигурацию в YAML")?;
			...
			Event {
				...
				commands: vec![
					Command {
						...
						description: Some("Получение последних изменений из репозитория".to_string()),
						timeout: Some(60),
						ignore_errors: None,
					},
					Command {
...
pub fn verify_deployment(config: &Config, deployment_name: &str) -> Result<bool> {
	...
	for event in &deployment.events {
		if event.commands.is_empty() {
			error!("Событие '{}' в деплое '{}' не содержит команд",
